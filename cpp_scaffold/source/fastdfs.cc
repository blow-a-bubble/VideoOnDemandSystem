...
namespace bubble 
{
    void FastDFSClient::init(const FastDFSSettings &settings) 
    {
        ...
        if (ret != 0) {
            ERROR__LOG("初始化全局配置失败: {}", STRERROR(ret));
            abort();
        }
    }
    void FastDFSClient::destroy() {
        fdfs_client_destroy();
    }
    std::optional<std::string> FastDFSClient::upload_from_buff(const std::string &buff) {
        auto tracker_server = tracker_get_connection();
        if (tracker_server == nullptr) {
            ERROR__LOG("获取tracker服务器连接句柄失败");
            return std::nullopt;
        }
        char file_id[256];
        int ret = storage_upload_by_filebuff1(tracker_server, nullptr, 0, buff.c_str(), buff.size(), nullptr, nullptr, 0, nullptr, file_id);
        if (ret!= 0) {
            ERROR__LOG("文件上传失败: {}", STRERROR(ret));
            return std::nullopt;
        }
    ...
    std::optional<std::string> FastDFSClient::upload_from_file(const std::string &path) {
        auto tracker_server = tracker_get_connection();
        if (tracker_server == nullptr) {
            ERROR__LOG("获取tracker服务器连接句柄失败");
            return std::nullopt;
    ...
    bool FastDFSClient::download_to_buff(const std::string &file_id, std::string &buff) {
        auto tracker_server = tracker_get_connection();
        if (tracker_server == nullptr) {
            ERROR__LOG("获取tracker服务器连接句柄失败");
            return false;
        ...
        if (ret!= 0) {
            ERROR__LOG("文件下载失败: {}", STRERROR(ret));
            return false;
        }
    ...
    bool FastDFSClient::download_to_file(const std::string &file_id, const std::string &path) {
        auto tracker_server = tracker_get_connection();
        if (tracker_server == nullptr) {
            ERROR__LOG("获取tracker服务器连接句柄失败");
            return false;
    ...
    bool FastDFSClient::remove(const std::string &file_id) {
        auto tracker_server = tracker_get_connection();
        if (tracker_server == nullptr) {
            ERROR__LOG("获取tracker服务器连接句柄失败");
            return false;
        ...
        if (ret != 0) {
            ERROR__LOG("文件删除失败: {}", STRERROR(ret));
            return false;
        }
        tracker_close_connection(tracker_server);
        return true;
